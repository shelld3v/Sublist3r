...
def write_file(filename, subdomains):
    ...
    print('%s[-] Saving results to file: %s%s%s%s' % (Y, W, R,
            filename, W))
...
class enumratorBase(object):
    ...
    def print_banner(self):
        ...
        self.print_(G + '[-] Searching now in %s..' % self.engine_name
                    + W)
        return
    ...
    def get_response(self, response):
        if response is None:
            return 0
        return (response.text if hasattr(response, 'text'
...
class GoogleEnum(enumratorBaseThreaded):
    ...
    def generate_query(self):
        if self.subdomains:
            ...
            query = \
                'site:{domain} -www.{domain}'.format(domain=self.domain)
        return query
...
class YahooEnum(enumratorBaseThreaded):
    ...
    def extract_domains(self, resp):
        link_regx2 = \
            re.compile('<span class=" fz-.*? fw-m fc-12th wr-bw.*?">(.*?)</span>'
                       )
        link_regx = \
            re.compile('<span class="txt"><span class=" cite fw-xl fz-15px">(.*?)</span>'
                       )
        links_list = []
        try:
        ...
        try:
            ...
            for link in links_list:
                if not link.startswith('http'):
                    link = 'http://' + link
                subdomain = urlparse.urlparse(link).netloc
                if subdomain not in self.subdomains and subdomain \
                    != self.domain:
                    if self.verbose:
                        self.print_('%s%s: %s%s' % (R,
                                    self.engine_name, W, subdomain))
...
class BingEnum(enumratorBaseThreaded):
    ...
    def extract_domains(self, resp):
        ...
        link_regx2 = \
            re.compile('<div class="b_title"><h2><a href="(.*?)"')
        try:
            links = link_regx.findall(resp)
            links2 = link_regx2.findall(resp)
            links_list = links + links2
    ...
    def generate_query(self):
        if self.subdomains:
            ...
            query = \
                'domain:{domain} -www.{domain}'.format(domain=self.domain)
        return query
...
class BaiduEnum(enumratorBaseThreaded):
    ...
    def generate_query(self):
        if self.subdomains and self.querydomain != self.domain:
            ...
            query = \
                'site:{domain} -site:www.{domain} -site:{found} '.format(domain=self.domain,
                    found=found)
        else:
            query = \
                'site:{domain} -site:www.{domain}'.format(domain=self.domain)
        return query
...
class NetcraftEnum(enumratorBaseThreaded):
    ...
    def create_cookies(self, cookie):
        ...
        cookies['netcraft_js_verification_response'] = \
            hashlib.sha1(urllib.unquote(cookies_list[1]).encode('utf-8'
                         )).hexdigest()
        return cookies
    ...
    def extract_domains(self, resp):
        links_list = list()
        link_regx = \
            re.compile('<a class="results-table__host" href="(.*?)"')
        try:
            links_list = link_regx.findall(resp)
            for link in links_list:
                subdomain = urlparse.urlparse(link).netloc
...
class DNSdumpster(enumratorBaseThreaded):
    ...
    def check_host(self, host):
        ...
        try:
            ...
            if ip:
                if self.verbose:
                    self.print_('%s%s: %s%s' % (R, self.engine_name, W,
                                host))
                is_valid = True
    ...
    def get_csrftoken(self, resp):
        csrf_regex = \
            re.compile('<input type="hidden" name="csrfmiddlewaretoken" value="(.*?)">'
                       , re.S)
    ...
    def enumerate(self):
        ...
        for subdomain in self.subdomains:
            t = threading.Thread(target=self.check_host,
                                 args=(subdomain, ))
            t.start()
            t.join()
    ...
    def extract_domains(self, resp):
        tbl_regex = \
            re.compile('<a name="hostanchor"><\/a>Host Records.*?<table.*?>(.*?)</table>'
                       , re.S)
    ...
    def extract_domains(self, resp):
        ...
        try:
            ...
            for link in links:
                ...
                for subdomain in subdomains:
                    if not subdomain.endswith(self.domain) or '*' \
                        in subdomain:
                        continue
...
def main(
    ...
    domain_check = \
        re.compile("^(http|https)?[a-zA-Z0-9]+([\-\.]{1}[a-zA-Z0-9]+)*\.[a-zA-Z]{2,}$"
                   )
    ...
    if not domain.startswith('http://') \
        or not domain.startswith('https://'):
        domain = 'http://' + domain
    ...
    if enable_bruteforce:
        ...
        resolvers = os.path.join(path_to_file, 'subbrute',
                                 'resolvers.txt')
